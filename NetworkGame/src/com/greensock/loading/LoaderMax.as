 ...
 */	
	public class LoaderMax extends LoaderCore {		
		/** @private **/
		public static const version:Number = 1.896;
...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		protected function _loadNext(event:Event=null):void {
			...
			if (_status == LoaderStatus.LOADING) {
				...
				var loader:LoaderCore, loaders:Array = _loaders.slice(), l:int = loaders.length, activeCount:uint = 0; //use _loaders.slice() because in some rare situations, a loader's COMPLETE event might occur immediately and if autoDispose is true, the length of the array could change mid-loop causing a skip.
				_calculateProgress();
				for (var i:int = 0; i < l; i++) {
					loader = loaders[i];
